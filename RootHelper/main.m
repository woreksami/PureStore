...
void setTSURLSchemeState(BOOL newState, NSString* customAppPath)
{
	...
	if(newState != getTSURLSchemeState(pathToUse))
	{
		...
		if(newState)
		{
			trollStoreInfoDictM[@"CFBundleURLTypes"] = PSURLScheme();
		}
		else
		{
...
{
	// Since PureStore Lite depends on ldid, we assume it exists
	return YES;
}
...
#else
			// Since PureStore Lite adhoc signs stuff, this means that on PMAP_CS devices, it will run with "PMAP_CS_IN_LOADED_TRUST_CACHE" trust level
...
// 171: a non purestore app with the same identifier is already installled
...
int installApp(NSString* appPackagePath, BOOL sign, BOOL force, BOOL isTSUpdate, BOOL useInstalldMethod, BOOL skipUICache)
{
	...
	if(([appId.lowercaseString isEqualToString:@"com.purestore.purestore"] && !isTSUpdate) || [immutableAppBundleIdentifiers() containsObject:appId.lowercaseString])
	{
		return 179;
	}
	...
	if(appContainer)
	{
		...
		// Make sure the installed app is a PureStore app or the container is empty (or the force flag is set)
		...
		{
			NSLog(@"[installApp] already installed and not a PureStore app... bailing out");
			return 171;
		}
		else if (appBundleURL) {
			// When overwriting an app that has been installed with a different PureStore flavor, make sure to remove the marker of said flavor
		...
		if(!isTSUpdate)
		{
			BKSTerminateApplicationForReasonAndReportWithDescription(appId, 5, false, @"PureStore - App updated");
		}
	...
	if(![[NSFileManager defaultManager] fileExistsAtPath:trollStoreMarkURL.path])
	{
		...
		if(!marked)
		{
			NSLog(@"[installApp] failed to mark %@ as PureStore app by creating %@, error: %@", appId, trollStoreMarkURL.path, creationError);
			return 177;
		}
	}
...
int uninstallPureStore(BOOL unregister)
{
...
int installPureStore(NSString* pathToTar)
{
	...
	NSString* tmpPureStorePath = [tmpPayloadPath stringByAppendingPathComponent:@"PureStore.app"];
	if(![[NSFileManager defaultManager] fileExistsAtPath:tmpPureStorePath]) return 1;

	//if (@available(iOS 16, *)) {} else {
		// Transfer existing ldid installation if it exists
		// But only if the to-be-installed version of PureStore is 1.5.0 or above
		// This is to make it possible to downgrade to older versions still

		NSString* toInstallInfoPlistPath = [tmpPureStorePath stringByAppendingPathComponent:@"Info.plist"];
		...
		if(result != NSOrderedDescending)
		{
			...
			{
				NSString* tmpLdidPath = [tmpPureStorePath stringByAppendingPathComponent:@"ldid"];
			...
			{
				NSString* tmpLdidVersionPath = [tmpPureStorePath stringByAppendingPathComponent:@"ldid.version"];
	...
	{
		setTSURLSchemeState(NO, tmpPureStorePath);
	}
	...
	{
		NSString* trollStorePersistenceHelper = [tmpPureStorePath stringByAppendingPathComponent:@"PersistenceHelper"];
		NSString* trollStoreRootHelper = [tmpPureStorePath stringByAppendingPathComponent:@"purestorehelper"];
	...
	NSLog(@"[installPureStore] installApp => %d", ret);
	[[NSFileManager defaultManager] removeItemAtPath:tmpPackagePath error:nil];
	return ret;
}
...
BOOL _installPersistenceHelper(LSApplicationProxy* appProxy, NSString* sourcePersistenceHelper, NSString* sourceRootHelper)
{
	...
	NSString* markPath = [bundlePath stringByAppendingPathComponent:@".PureStorePersistenceHelper"];
	NSString* rootHelperPath = [bundlePath stringByAppendingPathComponent:@"purestorehelper"];
...
void installPersistenceHelper(NSString* systemAppId, NSString *persistenceHelperBinary, NSString *rootHelperBinary)
{
	...
	if (rootHelperBinary == nil) {
		rootHelperBinary = [trollStoreAppPath() stringByAppendingPathComponent:@"purestorehelper"];
	}
	...
	BKSTerminateApplicationForReasonAndReportWithDescription(systemAppId, 5, false, @"PureStore - Reload persistence helper");
}
...
void unregisterUserPersistenceHelper()
{
	...
	if(userAppProxy)
	{
		NSString* markPath = [userAppProxy.bundleURL.path stringByAppendingPathComponent:@".PureStorePersistenceHelper"];
...
void uninstallPersistenceHelper(void)
{
	...
	if(systemAppProxy)
	{
		...
		NSString* helperPath = [bundlePath stringByAppendingPathComponent:@"purestorehelper"];
		NSString* markPath = [bundlePath stringByAppendingPathComponent:@".PureStorePersistenceHelper"];
		...
		BKSTerminateApplicationForReasonAndReportWithDescription(systemAppProxy.bundleIdentifier, 5, false, @"PureStore - Reload persistence helper");
	}
...
void registerUserPersistenceHelper(NSString* userAppId)
{
	...
	NSString* markPath = [appProxy.bundleURL.path stringByAppendingPathComponent:@".PureStorePersistenceHelper"];
	[[NSFileManager defaultManager] createFileAtPath:markPath contents:[NSData data] attributes:nil];
}

// Apparently there is some odd behaviour where PureStore installed apps sometimes get restricted
...
int MAIN_NAME(int argc, char *argv[], char *envp[])
{
	@autoreleasepool {
		...
		if(getuid() != 0)
		{
			NSLog(@"ERROR: purestorehelper has to be run as root.");
			return -1;
		}
		...
		NSLog(@"purestorehelper invoked with arguments: %@", args);
		...
		NSLog(@"purestorehelper returning %d", ret);
		return ret;
	}
}
