...
NSString* rootHelperPath(void)
{
	return [[NSBundle mainBundle].bundlePath stringByAppendingPathComponent:@"purestorehelper"];
}
#endif
...
int spawnRoot(NSString* path, NSArray* args, NSString** stdOut, NSString** stdErr)
{
	...
	if(stdOut || stdErr)
	{
		logQueue = dispatch_queue_create("com.purestore.PureStore.LogCollector", NULL);
...
void fetchLatestPureStoreVersion(void (^completionHandler)(NSString* latestVersion))
{
	github_fetchLatestVersion(@"opa334/PureStore", completionHandler);
}
...
NSArray* trollStoreInstalledAppContainerPathsInternal(NSString *marker)
{
	...
	for(NSString* container in containers)
	{
		...
		if(exists && isDirectory)
		{
			...
			{
				NSString* trollStoreApp = [containerPath stringByAppendingPathComponent:@"PureStore.app"];
				NSString* trollStoreLiteApp = [containerPath stringByAppendingPathComponent:@"PureStoreLite.app"];
...
{
	return [trollStorePath() stringByAppendingPathComponent:@"PureStore.app"];
}
...
LSApplicationProxy* findPersistenceHelperApp(PERSISTENCE_HELPER_TYPE allowedTypes)
{
	...
	void (^searchBlock)(LSApplicationProxy* appProxy) = ^(LSApplicationProxy* appProxy)
	{
		if(appProxy.installed && !appProxy.restricted)
		{
			...
			{
				NSURL* trollStorePersistenceMarkURL = [appProxy.bundleURL URLByAppendingPathComponent:@".PureStorePersistenceHelper"];
...
{
    NSObject *tsPreAppliedExploitType = infoDict[@"PSPreAppliedExploitType"];
    if([tsPreAppliedExploitType isKindOfClass:[NSNumber class]])
    {
        ...
        {
            NSLog(@"[getDeclaredExploitTypeFromInfoDictionary] rejecting PSPreAppliedExploitType Info.plist value (%i) which is out of range", exploitTypeInt);
        }
    }

    // Legacy Info.plist flag - now deprecated, but we treat it as a custom root cert if present
    NSObject *tsBundleIsPreSigned = infoDict[@"PSBundlePreSigned"];
