...
#import "PSInstallationController.h"
#import "PSSettingsAdvancedListController.h"
#import "PSDonateListController.h"

@interface NSUserDefaults (Private)
- (instancetype)_initWithSuiteName:(NSString *)suiteName container:(NSURL *)container;
@end
extern NSUserDefaults* pureStoreUserDefaults(void);

@implementation PSSettingsListController

- (void)viewDidLoad
{
	[super viewDidLoad];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadSpecifiers) name:UIApplicationWillEnterForegroundNotification object:nil];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadSpecifiers) name:@"PureStoreReloadSettingsNotification" object:nil];
...
- (NSMutableArray*)specifiers
{
	if(!_specifiers)
	{
		...
		if(_newerVersion)
		{
			PSSpecifier* updatePureStoreGroupSpecifier = [PSSpecifier emptyGroupSpecifier];
			updatePureStoreGroupSpecifier.name = @"Update Available";
			[_specifiers addObject:updatePureStoreGroupSpecifier];
		...
		//if (@available(iOS 16, *)) { } else {
			...
			if(ldidInstalled)
			{
				[signingGroupSpecifier setProperty:@"ldid is installed and allows PureStore to install unsigned IPA files." forKey:@"footerText"];
			}
			else
			{
				[signingGroupSpecifier setProperty:@"In order for PureStore to be able to install unsigned IPAs, ldid has to be installed using this button. It can't be directly included in PureStore because of licensing issues." forKey:@"footerText"];
			}
		...
		if([[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/PureStorePersistenceHelper.app"])
		{
			[persistenceGroupSpecifier setProperty:@"When iOS rebuilds the icon cache, all PureStore apps including PureStore itself will be reverted to \"User\" state and either disappear or no longer launch. If that happens, you can use the PureHelper app on the home screen to refresh the app registrations, which will make them work again." forKey:@"footerText"];
		...
		else
		{
			...
			if(persistenceApp)
			{
				...
				[persistenceGroupSpecifier setProperty:[NSString stringWithFormat:@"When iOS rebuilds the icon cache, all PureStore apps including PureStore itself will be reverted to \"User\" state and either disappear or no longer launch. If that happens, you can use the persistence helper installed into %@ to refresh the app registrations, which will make them work again.", appName] forKey:@"footerText"];
			...
			{
				[persistenceGroupSpecifier setProperty:@"When iOS rebuilds the icon cache, all PureStore apps including PureStore itself will be reverted to \"User\" state and either disappear or no longer launch. The only way to have persistence in a rootless environment is to replace a system application, here you can select a system app to replace with a persistence helper that can be used to refresh the registrations of all PureStore related apps in case they disappear or no longer launch." forKey:@"footerText"];
		...
		[installationSettingsGroupSpecifier setProperty:@"The URL Scheme, when enabled, will allow apps and websites to trigger PureStore installations through the apple-magnifier://install?url=<IPA_URL> URL scheme and enable JIT through the apple-magnifier://enable-jit?bundle-id=<BUNDLE_ID> URL scheme." forKey:@"footerText"];
		...
		PSSpecifier* advancedLinkSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Advanced"
										...
										cell:PSLinkListCell
										edit:nil];
		advancedLinkSpecifier.detailControllerClass = [PSSettingsAdvancedListController class];
		...
		PSSpecifier* donateSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Donate"
										...
										cell:PSLinkListCell
										edit:nil];
		donateSpecifier.detailControllerClass = [PSDonateListController class];
		...
		uninstallPureStoreSpecifier.buttonAction = @selector(doTheDashPressed);
...
{
	[PSInstallationController installLdid];
}
...
- (void)enableDevModePressed
{
	...
	if (ret == 0) {
		...
		[PSPresentationDelegate presentViewController:rebootNotification animated:YES completion:nil];
	} else {
...
- (void)installPersistenceHelperPressed
{
	...
	UIAlertController* selectAppAlert = [UIAlertController alertControllerWithTitle:@"Select App" message:@"Select a system app to install the PureStore Persistence Helper into. The normal function of the app will not be available, so it is recommended to pick something useless like the Tips app." preferredStyle:UIAlertControllerStyleActionSheet];
	...
	[PSPresentationDelegate presentViewController:selectAppAlert animated:YES completion:nil];
}
...
- (void)transferAppsPressed
{
	...
	{
		[PSPresentationDelegate startActivity:@"Transfering"];
	...
	[PSPresentationDelegate presentViewController:confirmationAlert animated:YES completion:nil];
}
...
- (void)setURLSchemeEnabled:(id)value forSpecifier:(PSSpecifier*)specifier
{
	...
	[PSPresentationDelegate presentViewController:rebuildNoticeAlert animated:YES completion:nil];
}
...
{
	NSUserDefaults* tsDefaults = pureStoreUserDefaults();
...
{
	NSMutableArray* args = @[@"uninstall-purestore"].mutableCopy;

	NSNumber* uninstallationMethodToUseNum = [pureStoreUserDefaults() objectForKey:@"uninstallationMethod"];
