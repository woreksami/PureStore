...
#import <PSUtil.h>
extern NSUserDefaults* pureStoreUserDefaults();

@implementation PSApplicationsManager

+ (instancetype)sharedInstance
{
    static PSApplicationsManager *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[PSApplicationsManager alloc] init];
    });
...
- (NSError*)errorForCode:(int)code
{
    ...
    switch(code)
    {
        ...
        break;
        case 177:
        errorDescription = @"Failed to mark app as PureStore app.";
        break;
        case 178:
    ...
    NSError* error = [NSError errorWithDomain:PureStoreErrorDomain code:code userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
    return error;
}
...
- (int)installIpa:(NSString*)pathToIpa force:(BOOL)force log:(NSString**)logOut
{
    ...
    NSNumber* installationMethodToUseNum = [pureStoreUserDefaults() objectForKey:@"installationMethod"];
