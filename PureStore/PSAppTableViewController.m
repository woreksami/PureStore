...
#import "PSApplicationsManager.h"
...
@implementation PSAppTableViewController

- (void)loadAppInfos
{
	NSArray* appPaths = [[PSApplicationsManager sharedInstance] installedAppPaths];
	NSMutableArray<PSAppInfo*>* appInfos = [NSMutableArray new];

	for(NSString* appPath in appPaths)
	{
		PSAppInfo* appInfo = [[PSAppInfo alloc] initWithAppBundlePath:appPath];
	...
	{
		[appInfos enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(PSAppInfo* appInfo, NSUInteger idx, BOOL* stop)
		{
	...
	[appInfos sortUsingComparator:^(PSAppInfo* appInfoA, PSAppInfo* appInfoB)
	{
...
- (void)_setUpNavigationBar
{
	UIAction* installFromFileAction = [UIAction actionWithTitle:@"Install IPA File" image:[UIImage systemImageNamed:@"doc.badge.plus"] identifier:@"InstallIPAFile" handler:^(__kindof UIAction *action)
	{
		dispatch_async(dispatch_get_main_queue(), ^
		{
			...
			[PSPresentationDelegate presentViewController:documentPickerVC animated:YES completion:nil];
		});
	}];
	...
	UIAction* installFromURLAction = [UIAction actionWithTitle:@"Install from URL" image:[UIImage systemImageNamed:@"link.badge.plus"] identifier:@"InstallFromURL" handler:^(__kindof UIAction *action)
	{
		dispatch_async(dispatch_get_main_queue(), ^
		{
			...
			UIAlertAction* installAction = [UIAlertAction actionWithTitle:@"Install" style:UIAlertActionStyleDefault handler:^(UIAlertAction* action)
			{
				...
				[PSInstallationController handleAppInstallFromRemoteURL:remoteURL completion:nil];
			}];
			...
			[PSPresentationDelegate presentViewController:installURLController animated:YES completion:nil];
		});
	}];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls
{
	...
	[PSInstallationController presentInstallationAlertIfEnabledForFile:pathToIPA isRemoteInstall:NO completion:nil];
}

- (void)openAppPressedForRowAtIndexPath:(NSIndexPath*)indexPath enableJIT:(BOOL)enableJIT
{
	PSApplicationsManager* appsManager = [PSApplicationsManager sharedInstance];

	PSAppInfo* appInfo = _cachedAppInfos[indexPath.row];
	...
	if(!didOpen)
	{
		...
		[PSPresentationDelegate presentViewController:didFailController animated:YES completion:nil];
	}
	else if (enableJIT)
	{
		...
		if (ret != 0)
		{
			UIAlertController* errorAlert = [UIAlertController alertControllerWithTitle:@"Error" message:[NSString stringWithFormat:@"Error enabling JIT: purestorehelper returned %d", ret] preferredStyle:UIAlertControllerStyleAlert];
	...
	[appInfo loadInfoWithCompletion:^(NSError* error)
	{
		dispatch_async(dispatch_get_main_queue(), ^
		{
			if(!error)
			{
				...
				[PSPresentationDelegate presentViewController:detailsAlert animated:YES completion:nil];
			}
			else
			{
...
- (void)changeAppRegistrationForRowAtIndexPath:(NSIndexPath*)indexPath toState:(NSString*)newState
{
	...
	if([newState isEqualToString:@"User"])
	{
		...
		UIAlertController* confirmationAlert = [UIAlertController alertControllerWithTitle:title message:@"Switching this app to a \"User\" registration will make it unlaunchable after the next respring because the bugs exploited in PureStore only affect apps registered as \"System\".\nThe purpose of this option is to make the app temporarily show up in settings, so you can adjust the settings and then switch it back to a \"System\" registration (PureStore installed apps do not show up in settings otherwise). Additionally, the \"User\" registration state is also useful to temporarily fix iTunes file sharing, which also doesn't work for PureStore installed apps otherwise.\nWhen you're done making the changes you need and want the app to become launchable again, you will need to switch it back to \"System\" state in PureStore." preferredStyle:UIAlertControllerStyleAlert];
		...
		{
			[[PSApplicationsManager sharedInstance] changeAppRegistration:[appInfo bundlePath] toState:newState];
			[appInfo sync_loadBasicInfo];
		}];
	...
	else
	{
		...
		[PSPresentationDelegate presentViewController:infoAlert animated:YES completion:nil];
	}
}
...
- (void)uninstallPressedForRowAtIndexPath:(NSIndexPath*)indexPath
{
	...
	[PSPresentationDelegate presentViewController:confirmAlert animated:YES completion:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	[PSPresentationDelegate presentViewController:appSelectAlert animated:YES completion:nil];
}
