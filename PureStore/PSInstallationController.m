...
@implementation PSInstallationController
...
+ (void)handleAppInstallFromFile:(NSString*)pathToIPA forceInstall:(BOOL)force completion:(void (^)(BOOL, NSError*))completionBlock
{
	...
	{
		[PSPresentationDelegate startActivity:@"Installing"];
		dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
		{
			// Install IPA
			NSString* log;
			int ret = [[PSApplicationsManager sharedInstance] installIpa:pathToIPA force:force log:&log];

			NSError* error;
			if(ret != 0)
			{
				error = [[PSApplicationsManager sharedInstance] errorForCode:ret];
			}
			...
			dispatch_async(dispatch_get_main_queue(), ^
			{
				[PSPresentationDelegate stopActivityWithCompletion:^
				{
					if (ret == 0) {
						...
						[PSPresentationDelegate presentViewController:warningAlert animated:YES completion:nil];
					} else {
						// unrecoverable error
						UIAlertController* errorAlert = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"Install Error %d", ret] message:[error localizedDescription] preferredStyle:UIAlertControllerStyleAlert];
...
{
	NSNumber* installAlertConfigurationNum = [pureStoreUserDefaults() objectForKey:@"installAlertConfiguration"];
	...
	PSAppInfo* appInfo = [[PSAppInfo alloc] initWithIPAPath:pathToIPA];
	[appInfo loadInfoWithCompletion:^(NSError* error)
	{
		dispatch_async(dispatch_get_main_queue(), ^
		{
			if(!error)
			{
				...
				[PSPresentationDelegate presentViewController:installAlert animated:YES completion:nil];
			}
			else
			{
...
+ (void)handleAppInstallFromRemoteURL:(NSURL*)remoteURL completion:(void (^)(BOOL, NSError*))completionBlock
{
	...
	dispatch_async(dispatch_get_main_queue(), ^
	{
			...
			dispatch_async(dispatch_get_main_queue(), ^
			{
				[PSPresentationDelegate stopActivityWithCompletion:^
				{
					if(error)
					{
						...
						[PSPresentationDelegate presentViewController:errorAlert animated:YES completion:^
						{
							if(completionBlock) completionBlock(NO, error);
						}];
					}
		...
		[PSPresentationDelegate startActivity:@"Downloading" withCancelHandler:^
		{
			[downloadTask cancel];
		}];
...
+ (void)installLdid
{
	fetchLatestLdidVersion(^(NSString* latestVersion)
	{
		...
		dispatch_async(dispatch_get_main_queue(), ^
		{
			...
			[PSPresentationDelegate startActivity:@"Installing ldid"];
			...
			NSURLSessionDownloadTask* downloadTask = [NSURLSession.sharedSession downloadTaskWithRequest:ldidRequest completionHandler:^(NSURL *location, NSURLResponse *response, NSError *error)
			{
				...
				else if(location)
				{
					...
					dispatch_async(dispatch_get_main_queue(), ^
					{
						...
						[[NSNotificationCenter defaultCenter] postNotificationName:@"PureStoreReloadSettingsNotification" object:nil userInfo:nil];
					});
				}
			}];
