...
@implementation PSSceneDelegate
	...
	for(UIOpenURLContext* context in URLContexts)
	{
		...
		if(url)
		{
			if([url isFileURL])
			{
				...
				{
					[PSInstallationController presentInstallationAlertIfEnabledForFile:url.path isRemoteInstall:NO completion:^(BOOL success, NSError* error){
						doneBlock(NO);
					}];
				}
				else if([url.pathExtension.lowercaseString isEqualToString:@"tar"])
				{
					// Update PureStore itself
					NSLog(@"Updating PureStore...");
					int ret = spawnRoot(rootHelperPath(), @[@"install-purestore", url.path], nil, nil);
					doneBlock(ret == 0);
					NSLog(@"Updated PureStore!");
				}
			}
			else if([url.scheme isEqualToString:@"apple-magnifier"])
			{
				...
				if([components.host isEqualToString:@"install"])
				{
					...
					if(URLStringToInstall && [URLStringToInstall isKindOfClass:NSString.class])
					{
						...
						[PSInstallationController handleAppInstallFromRemoteURL:URLToInstall completion:nil];
					}
				}
...
- (void)handleEnableJITForBundleID:(NSString *)appId
{
	...
	if(!didOpen)
	{
		...
		// we don't have PSAppInfo here so we cannot check the registration state
...
// or if it's the one from an old PureStore version that's no longer supported
...
- (void)scene:(UIScene *)scene willConnectToSession:(UISceneSession *)session options:(UISceneConnectionOptions *)connectionOptions {
	...
	_rootViewController = [[PSRootViewController alloc] init];
