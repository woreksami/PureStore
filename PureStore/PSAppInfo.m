#import "PSAppInfo.h"
#import "PSCommonTCCServiceNames.h"
#import <PSUtil.h>
	...
	if(!_cachedAppBundleName)
	{
		if(_isArchive)
		{
			...
			if(!_cachedAppBundleName)
			{
				...
				outError = [NSError errorWithDomain:PureStoreErrorDomain code:301 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
			}
		}
	}
...
- (NSError*)loadInfoDictionary
{
	...
	if(!_cachedInfoDictionary)
	{
		...
		return [NSError errorWithDomain:PureStoreErrorDomain code:302 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
	}
		...
		if(_isArchive)
		{
			if(_cachedInfoDictionary)
			{
				...
				if(!mainBinaryEntry)
				{
					...
					return [NSError errorWithDomain:PureStoreErrorDomain code:303 userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
				}
